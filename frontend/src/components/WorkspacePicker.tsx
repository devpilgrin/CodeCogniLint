    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
      <div className="bg-[#161b22] border border-[#30363d] rounded-xl shadow-2xl w-full max-w-2xl flex flex-col" style={{ maxHeight: '85vh' }}>
        ...
        <div className="flex border-b border-[#30363d] px-4">
          {([
            { id: 'local',  label: 'Локальная папка', icon: faFolderOpen },
            { id: 'git',    label: 'Git-репозиторий', icon: faCodeBranch },
            { id: 'recent', label: `Недавние (${recent.length})`, icon: faClockRotateLeft },
          ] as const).map(t => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
        ...
        <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
          ...
          {tab === 'local' && (
            <div className="space-y-4">
              <p className="text-xs text-gray-400">
                Укажите абсолютный путь к папке с проектом на этом компьютере.
              </p>
              <div>
                <label className="text-[10px] text-gray-500 uppercase font-semibold block mb-1">Путь</label>
          ...
          {tab === 'git' && (
            <div className="space-y-4">
              <p className="text-xs text-gray-400">
                Введите URL — репозиторий будет клонирован в <code className="bg-[#0d1117] px-1 rounded text-gray-300">backend/projects/</code>.
              </p>
              <div>
                <label className="text-[10px] text-gray-500 uppercase font-semibold block mb-1">URL репозитория</label>
                <input
                  type="text"
                  value={gitUrl}
              ...
              <div>
                <label className="text-[10px] text-gray-500 uppercase font-semibold block mb-1">
                  Имя папки <span className="text-gray-600 font-normal normal-case">(необязательно)</span>
                </label>
                <input
                  type="text"
                  value={gitTarget}
                  onChange={e => setGitTarget(e.target.value)}
                  placeholder="по умолчанию — из URL"
                  className="w-full bg-[#0d1117] border border-[#30363d] rounded px-3 py-2 text-xs text-gray-200 focus:outline-none focus:border-blue-500 code-font"
                />
              </div>
              <button
              ...
              >
                ...
                {loading ? 'Клонирование...' : 'Клонировать и открыть'}
              </button>
            </div>
          )}
          ...
          {tab === 'recent' && (
            <div className="space-y-1">
              {recent.length === 0 && (
                <p className="text-xs text-gray-500 text-center py-8">Нет недавних проектов</p>
              )}
              {recent.map(path => (
                <button
                  key={path}
        ...
        <div className="fixed inset-0 bg-black/50 z-[210] flex items-center justify-center p-4" onClick={() => setBrowseOpen(false)}>
          <div className="bg-[#161b22] border border-[#30363d] rounded-xl shadow-2xl w-full max-w-md flex flex-col" style={{ maxHeight: '70vh' }} onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-3 border-b border-[#30363d]">
              <span className="text-sm font-bold text-white">Выбор папки</span>
            ...
            <div className="px-3 py-2 bg-[#0d1117] border-b border-[#30363d] flex items-center space-x-2">
              {browser.parent && (
                <button onClick={() => openBrowse(browser.parent!)} className="text-gray-400 hover:text-white text-xs p-1" title="Вверх">
                  <FontAwesomeIcon icon={faArrowUp} />
                </button>
              )}
              <button onClick={() => openBrowse(browser.home)} className="text-gray-400 hover:text-white text-xs p-1" title="Домой">
                <FontAwesomeIcon icon={faHouse} />
              </button>
              <span className="text-[11px] text-gray-300 code-font truncate flex-1">{browser.current || 'Корень'}</span>
            </div>
            <div className="flex-1 overflow-y-auto custom-scrollbar p-2">
              {browser.entries.length === 0 ? (
                <p className="text-xs text-gray-500 text-center py-4">Папок нет</p>
              ) : (
